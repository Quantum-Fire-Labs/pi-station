...
import { isProjectWorkspaceMoveMutation, isWorkspaceCreateMutation, isWorkspaceState, isWorkspaceUpdateMutation } from "./workspaces.js"
...
describe("Workspace protocol", () => {
  it("accepts exclusive Project ownership and per-Workspace state", () => {
    ...
    expect(isProjectWorkspaceMoveMutation({ workspaceId: "workspace-2" })).toBe(true)
    expect(isWorkspaceState({ workspaces: [workspace], activeWorkspaceId: workspace.id })).toBe(true)
  })

  it("rejects membership edits and invalid ownership", () => {
