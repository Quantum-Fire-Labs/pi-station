...
service=${PI_STATION_SYSTEMD_SERVICE:-pi-station.service}
...
serve_route_existed=$(printf '%s' "$serve_status" | node -e '
let input = "";
process.stdin.on("data", (chunk) => { input += chunk; });
process.stdin.on("end", () => {
  const status = JSON.parse(input || "{}");
  ...
  if (status.TCP?.[port] === undefined) { process.stdout.write("0"); return; }
...
environment_backup="$work_directory/environment"
environment_replacement=$(mktemp "${environment_file}.new.XXXXXX")
...
rollback() {
  ...
  cp "$environment_backup" "$environment_replacement"
  chmod 600 "$environment_replacement"
  mv "$environment_replacement" "$environment_file"
  if [[ "$serve_route_existed" == "0" ]]; then tailscale serve --yes --https="$https_port" off >/dev/null 2>&1 || true; fi
  systemctl --user restart "$service" || true
}
trap 'status=$?; if (( status != 0 )); then rollback; fi; rm -f -- "$environment_replacement"; rm -rf -- "$work_directory"; exit "$status"' EXIT
...
node -e '
const fs = require("node:fs");
const [source, output, origin] = process.argv.slice(1);
const text = fs.readFileSync(source, "utf8");
const pattern = /^PI_STATION_WEB_ORIGIN=.*$/mu;
if (!pattern.test(text)) throw new Error("PI_STATION_WEB_ORIGIN is missing");
fs.writeFileSync(output, text.replace(pattern, `PI_STATION_WEB_ORIGIN="${origin}"`), { mode: 0o600 });
' "$environment_file" "$environment_replacement" "$web_origin" || fail "could not update the Pi Station origin"
mv "$environment_replacement" "$environment_file"
