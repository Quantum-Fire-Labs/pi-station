...
function fixture(serveStatus = "{}") {
  ...
  const dropins = resolve(root, "dropins")
...
`)
  ...
  return { root, bin, config, dropins, calls }
}
...
describe("Tailscale setup", () => {
  ...
  it("restores the prior origin and Serve configuration when remote validation fails", () => {
    const value = fixture()
    mkdirSync(value.dropins, { recursive: true })
    const dropin = resolve(value.dropins, "90-tailscale-origin.conf")
    writeFileSync(dropin, '[Service]\nEnvironment="PI_STATION_WEB_ORIGIN=https://old.example.ts.net"\n')
  ...
  it("does not replace an unrelated HTTPS route", () => {
    ...
    expect(() => readFileSync(resolve(value.dropins, "90-tailscale-origin.conf"))).toThrow()
  })
})
